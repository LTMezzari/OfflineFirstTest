package mezzari.torres.lucas.data.viacep.repository
 ...
 * @since 30/05/2023
 */
class AddressRepositoryImpl(private val api: ViacepAPI) :
    AddressRepository {
    override fun getAddress(cep: String): Flow<Resource<Address>> {
        return flow {
            DataBoundResource(this)
                .execute(OnlineStrategy({ api.getAddress(cep) }))
        }
    }
}